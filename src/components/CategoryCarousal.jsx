...
const colors = ["yellow-500", "red-600", "green-600"];
const borderColors = ["border-yellow-500", "border-red-600", "border-green-600"];
    ...
    >
      <div className="mb-8">
        <div className={`flex justify-between items-center bg-white border-t-2 ${borderColors[index]}`}>
          ...
          >
            See all
            <BiArrowBack className={`rotate-180 ml-1 text-sm md:text-lg fill-${colors[index]}`}/>
          </Link>
        </div>
      </div>
            ...
            data.map((item, ind) => {
              return (
                ...
                >
                  ...
                  <div className={`${textColors[index]} my-2 text-xl font-bold text-center`}>
                    {item[`${sellType}_name`]}
                  </div>
                </Link>
              );
